...
export const likeBlog = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to like blog" });
  }
};
//dislike blog
export const dislikeBlog = async (req, res) => {
  try {
    ...
    if (!blog.likes.includes(likeId)) {
      return res
        .status(400)
        .json({ message: "You haven't liked this blog", success: false });
    }

    blog.likes = blog.likes.filter((id) => id.toString() !== likeId.toString());
    await blog.save();
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to dislike blog" });
  }
};
...
export const getMyTotalBlogLikes = async (req, res) => {
  try {
    ...
    // Find all blogs authored by the logged-in user
    const myBlogs = await Blog.find({ author: userId }).select("likes");

    // Sum up the total likes
